...
- Other workarounds that some Penguins have had some success with:
   - Create a new `dxvk.conf` file and add `d3d11.cachedDynamicResources = "a"` to it, then export `DXVK_CONFIG_FILE=your/dxvk/config/location/dxvk.conf` when running the game. If using Lutris, configure it as a new environment variable in the game's settings.
