...
- Other workarounds that some Penguins have had some success with:
   - Set `d3d11.cachedDynamicResources = "a"` in a new, blank `dxvk.conf` file then export `DXVK_CONFIG_FILE=your/dxvk/config/location/dxvk.conf` when running the game. If using Lutris, configure it as a new environment variable in the game's settings.
